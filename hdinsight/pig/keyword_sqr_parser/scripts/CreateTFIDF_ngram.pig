...
TOKENS_BAG_f = FOREACH SQR_Tuples GENERATE campaign, adgroup, query, FLATTEN(ngram.forward_ngram(query, impressions,clicks));
TOKENS_BAG_b = FOREACH SQR_Tuples GENERATE campaign, adgroup, query, FLATTEN(ngram.reverse_ngram(query, impressions,clicks));

TOKENS_BAG_DUP = UNION TOKENS_BAG_f, TOKENS_BAG_b;
...
rmf /TFIDF_NGram
STORE TFIDF INTO '/TFIDF_NGram' USING PigStorage(',');